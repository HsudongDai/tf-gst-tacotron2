...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None, reference_mel=None, reference_weight=None):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      if reference_mel is not None:
        ...
        if hp.use_gst:
          ...
          style_weights, style_embeddings = style_attention.multi_head_attention()                   # [N, 1, 256]
        else:
        ...
        specific_weights = tf.tile(specific_weights, [hp.num_heads, 1], name="specific_weights")
        ...
        style_embeddings = tf.matmul(specific_weights, tf.nn.tanh(gst_tokens))
        style_embeddings = tf.expand_dims(style_embeddings, axis=0)
        style_embeddings = tf.tile(style_embeddings, [batch_size, 1, 1])
        style_embeddings = tf.reshape(style_embeddings, shape=[batch_size, 1, hp.style_embed_depth])
        ...
        random_weights = tf.nn.softmax(random_weights, axis=-1, name="random_weights")
