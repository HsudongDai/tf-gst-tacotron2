  ...
  def multi_head_attention(self):
    ...
    with tf.variable_scope("Multihead-attention"):
      ...
      if self.attention_type == 'mlp_attention':
        style_weights, style_embeddings = self._mlp_attention(qs, ks, vs)
      elif self.attention_type == 'dot_attention':
        style_weights, style_embeddings = self._dot_product(qs, ks, vs)
      else:
        raise ValueError('Only mlp_attention and dot_attention are supported')

      return tf.squeeze(style_weights, axis=2), self._combine_heads(style_embeddings)
