...
class DataFeeder(threading.Thread):
  ...
  def _get_next_example(self):
    ...
    input_data = np.asarray(text_to_sequence(meta[2], self._cleaner_names), dtype=np.int32)
    linear_target = np.load(os.path.join(self._datadir, "linear", f"{meta[0]}.npy")).T
    mel_target = np.load(os.path.join(self._datadir, "mel", f"{meta[0]}.npy")).T
